...
from orchestra.utils.python import format_exception, import_class
...
class Setting(object):
        ...
        for choice in value:
            ...
            if not isinstance(verbose, (str, Promise)):
                raise ValidationError(
                    "%s is not a valid verbose name." % value)
    ...
    def validate_non_empty_string(cls, value):
        non_empty_validator = RegexValidator(
            regex=r'^.+$',
            message="This field cannot be empty.",
            code='invalid_non_empty'
        )
        return non_empty_validator(value)
    ...
    def validate_value(self, value):
        ...
        if not isinstance(value, tuple(valid_types)):
            raise ValidationError("%s is not a valid type (%s)." %
                                  (type(value).__name__, ', '.join(
                                      t.__name__ for t in valid_types))
                                  )
